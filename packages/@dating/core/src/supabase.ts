...
export async function setUserLocation(tableName: string, userId: number, lat: number, lng: number): Promise<void> {
  if (!hasValidKey) return
  try {
    await fetch(`${SUPABASE_URL}/rest/v1/${tableName}?id=eq.${userId}`, {
      method: 'PATCH', headers,
  ...
  } catch (err) { console.error('setUserLocation:', err) }
}
...
export async function fetchUserById(tableName: string, userId: number): Promise<DbUser | null> {
  if (!hasValidKey) return null
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${tableName}?id=eq.${userId}&select=*&limit=1`, { headers })
    if (!res.ok) return null
    const data = await res.json() as DbUser[]
...
// ─── Fetch all users (no location filter) ────────────────────────────
export async function fetchAllUsers(tableName: string, limit = 200): Promise<DbUser[]> {
  if (!hasValidKey) return []
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${tableName}?select=*&limit=${limit}&order=updated_at.desc`, { headers })
    if (!res.ok) return []
